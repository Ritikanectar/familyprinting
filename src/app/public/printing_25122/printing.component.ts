  ...
  constructor(private http:HttpClient, private renderer:Renderer2,private router:Router, private route:ActivatedRoute, 
   ...
   open(content:any) {
    ...
    if(this.printLocation=='both' || this.printLocation=='front_only'){    
      ...
      if(this.text2X!=''){
        $(".dynaImgFC").append("<label style='left:"+(this.text2X)+"px;width:"+this.text2width+"px;top:"+(this.text2Y)+"px;font-size:"+this.text2Size+";color:"+this.text2Color+";font-family:"+this.textFamily2+";letter-spacing:"+this.textSpacing2+";font-weight:"+this.textweight2+";position:absolute;' >"+this.text2+"</label>");
      }
    ...
    this.http.post(this.url + 'get-product-details', { event_id: this.evID, user_id: this.userID, product_id: this.productID, psize: this.productSize, pColor: this.pColor }).subscribe((data: any) => {
      ...
      if(data.flag){
        ...
        $.each(data.prod_colors, function (k, rows: any) {
          console.log(rows.frontview_file);
        ...
        this.fCanvasWidth = data.product.canvas_front_width;
        this.fCanvasHeight = data.product.canvas_front_height;
        this.bCanvasWidth = data.product.canvas_back_width;
        this.bCanvasHeight = data.product.canvas_back_height;
        
        this.canvas.setWidth(data.product.canvas_front_width);
        this.canvas.setHeight(data.product.canvas_front_height);

        this.canvasBack.setWidth(data.product.canvas_back_width);
        this.canvasBack.setHeight(data.product.canvas_back_height);
