...
class AppController extends Controller
{
     ...
     public function create_event(Request $request){
       ...
       // $event->store_id = $jsonData['stores'];
        ...
        if($event->save()){
          ...
          $mail->Username   = "nilesh.vishwakarma@nectarinfotel.com";
          $mail->Password   = "bzxbjdckivwzgfwk";
    ...
    public function get_products(Request $request){
          ...
          $test = [];
          $product = Product::select('fp_product.product_id','fp_product.name','fp_product.canvas_front_width','fp_product.canvas_front_height','fp_product.productview_file','fp_product.productview_file_ext','fp_product.canvas_back_width','fp_product.canvas_back_height','pc.frontview_file','fp_product.backview_file',DB::raw('GROUP_CONCAT(sz.size ORDER BY sz.size) as size'),DB::raw('GROUP_CONCAT(pc.color ORDER BY pc.color) as color'),'tp.product as type','fp_product.other_size as other_size')
          ...
          $totalRecords = Product::where('fp_product.is_active',1)->count();
                   ...
                   // $test = \DB::getQueryLog();
          // $totalRecordwithFilter = Product::select('fp_product.id')
          //           ->leftJoin('fp_product_size', 'fp_product_size.product_id', '=', 'fp_product.product_id')
          //           ->leftJoin('fp_product_type', 'fp_product_type.type_id', '=', 'fp_product.type_id')
          //           ->where('fp_product.is_active',1)->groupBy('fp_product.product_id');          
                   /// echo $searchValue;die();
           //$test = $totalRecordwithFilter->count();
           if($searchValue != ''){
            $test = $searchQuery;
            $totalRecordwithFilter = Product::leftJoin('fp_product_size AS sz', 'sz.product_id', '=', 'fp_product.product_id')
                    ->leftJoin('fp_product_type AS tp', 'tp.type_id', '=', 'fp_product.type_id')
                    ->where('fp_product.is_active',1)->groupBy('fp_product.product_id');
            $totalRecordwithFilter = $totalRecordwithFilter->whereRaw($searchQuery);
            $totalRecordwithFilter = $totalRecordwithFilter->count();
           }else{
   ...
   public function get_product(Request $request){
      ...
      $productRes = Product::select('fp_product.product_id','fp_product.name','fp_product.type_id','fp_product.color','fp_product.canvas_front_width','fp_product.productview_file','fp_product.productview_file_ext','fp_product.canvas_front_height','fp_product.canvas_back_width','fp_product.canvas_back_height','fp_product.frontview_file','fp_product.backview_file',DB::raw('GROUP_CONCAT(sz.size ORDER BY sz.size) as size'),'tp.product as type','fp_product.other_size')
                ->leftjoin('fp_product_size AS sz', 'sz.product_id', '=', 'fp_product.product_id')
                ->join('fp_product_type AS tp', 'tp.type_id', '=', 'fp_product.type_id')
        ...
        foreach($clrRows as $rows){
            if($rows['frontview_file']!=''){
              ...
              $prodColors[$rows['color']]['front_canvas_left'] = $rows['front_canvas_left'];
              $prodColors[$rows['color']]['front_canvas_top'] = $rows['front_canvas_top'];

              $prodColors[$rows['color']]['front_canvas_width'] = $rows['front_canvas_width'];
              $prodColors[$rows['color']]['front_canvas_height'] = $rows['front_canvas_height'];
              $prodColors[$rows['color']]['resize_front_width'] = $rows['resize_front_width'];
              $prodColors[$rows['color']]['resize_front_height'] = $rows['resize_front_height'];
              $prodColors[$rows['color']]['front_canvas_transform'] = $rows['front_canvas_transform'];
            }
            if($rows['backview_file']!=''){
              $prodColors[$rows['color']]['back'] = $rows['backview_file'];
              $prodColors[$rows['color']]['color_name'] = $rows['color_name'];
              $prodColors[$rows['color']]['id'] = $rows['id'];
              $prodColors[$rows['color']]['back_canvas_left'] = $rows['back_canvas_left'];
              $prodColors[$rows['color']]['back_canvas_top'] = $rows['back_canvas_top'];

              $prodColors[$rows['color']]['back_canvas_width'] = $rows['back_canvas_width'];
              $prodColors[$rows['color']]['back_canvas_height'] = $rows['back_canvas_height'];
              $prodColors[$rows['color']]['resize_back_width'] = $rows['resize_back_width'];
              $prodColors[$rows['color']]['resize_back_height'] = $rows['resize_back_height'];
              $prodColors[$rows['color']]['back_canvas_transform'] = $rows['back_canvas_transform'];
    ...
    public function create_product(Request $request){
      ...
      $frontcanvasresize = json_decode($request->frontcanvasresize);
      $backcanvassize = json_decode($request->backcanvasresize);
      
      
      // echo '<pre>';
      // print_r($colors);
      // die();
      //return response()->json(['msg'=>$colors[0]->color,'file'=>$_FILES,'flag'=>TRUE],400);
      ...
      if($product->save()){ 
        ...
        if(isset($_FILES['frontview']) && !empty($_FILES['frontview'])){
            ...
            for($i=0;$i<$countfiles;$i++){
              ...
              $productClr->front_canvas_width = !empty($colors)? $colors[$i]->front_width:'';
              $productClr->front_canvas_height = !empty($colors)? $colors[$i]->front_height:'';
              $productClr->resize_front_width = !empty($colors)? $frontcanvasresize[$i]->width:'';
              $productClr->resize_front_height = !empty($colors)? $frontcanvasresize[$i]->height:'';

              $productClr->front_canvas_left = !empty($colors)? $frontcanvasresize[$i]->left:'';
              $productClr->front_canvas_top = !empty($colors)? $frontcanvasresize[$i]->top:'';
              $productClr->front_canvas_transform = !empty($colors)? $frontcanvasresize[$i]->transform:'';
              // $productClr->front_canvas_left = $request->canvas_front_left;
              // $productClr->front_canvas_top = $request->canvas_front_top;
          ...
          if(isset($_FILES['productimg']) && !empty($_FILES['productimg'])){
            ...
            Product::where('product_id', $product->product_id)
                ->update([
                    'productview_file' => $filePath.'/back_'.$file->getClientOriginalName(),
                    'productview_file_ext' => $ext
                  ]);
          }
        ...
        //       move_uploaded_file($_FILES['productimg']['tmp_name'][$i],'uploads/'.$filePath.'/back_'.$filename);
        //       $productClr = new ProductColors;
             
        //       $productClr->productview_file = $filePath.'/back_'.$filename;
        //       $productClr->backview_file_ext = $_FILES['productimg']['type'][$i];
          ...
          if(!empty($colors)){
            for($i=0;$i<count($colors);$i++){
              ...
              if(isset($_FILES['frontview']['name'][$i])){
                ...
                // return response()->json(['msg'=>'Product updated successfully','products'=> !empty($colors)? $colors[$i]->front_width:'','flag'=>FALSE],500);
                // echo '<pre>';
                // print_r($colors);
                // die('innn');
                    $pclr = ProductColors::where([['product_id','=',$request->product_id],['frontview_file','!=',''],['is_active','=',1]])->get();
    ...
    public function deactive_user(Request $request){
     ...
     public function get_layout_dd(Request $request){
      ...
      if($request->event_id){
          ...
          $products = Product::select('fp_product.product_id','fp_product.name','fp_product.canvas_front_width','fp_product.canvas_front_height','fp_product.productview_file','fp_product.canvas_back_width','fp_product.canvas_back_height','pc.front_canvas_width','pc.front_canvas_height','pc.back_canvas_width','pc.back_canvas_height','pc.frontview_file','fp_product.backview_file','fp_product.is_active',DB::raw('GROUP_CONCAT(sz.size ORDER BY sz.size) as size'),DB::raw('GROUP_CONCAT(pc.color ORDER BY pc.color) as color'),'tp.product as type','fp_product.other_size as other_size')
                ->leftjoin('fp_product_size AS sz', 'sz.product_id', '=', 'fp_product.product_id')
                ->join('fp_product_type AS tp', 'tp.type_id', '=', 'fp_product.type_id')
                ->join('fp_product_colors AS pc',function($join){
                      $join->on('pc.product_id', '=', 'fp_product.product_id');
    ...
    public function get_app_list(Request $request){
      ...
      $columnName = $request->columns[$columnIndex]['data']; // Column name
      $columnSortOrder = $request->order[0]['dir']; // asc or desc
      $searchValue = $request->search['value']; // Search value c
      $searchQuery = " ";
      if($searchValue != ''){
      ...
      foreach ($event as $key=>$rows) {
        $event[$key]['link'] = "http://fpuat.nectarinfotel.com/#/landing/".Crypt::encryptString($rows['event_id'])."/".Crypt::encryptString($rows['created_by']);
      }
    ...
    public function create_product_layout(Request $request){
      ...
      if($existProduct->count()>0){
        ...
        $productApp->header_size_text = $request->header_size_text;
...
// return response()->json(['msg'=>DB::getQueryLog(),'flag'=>TRUE],200);
    ...
    public function create_thankyou(Request $request){
      ...
      if($existThankyou->count()>0){
        ...
        return response()->json(['msg'=>'Thank You Layout Updated','testdt'=>$test,'flag'=>TRUE],200);
      }else{
